...
%	Description:
%
%	KERN = KERNTEST(KERNTYPE, NUMIN, TIEPARAMNAMES, FLAGS) runs some
%	tests on a kernel with the specified type to ensure it is correctly
%	implemented.
%	 Returns:
...
%	  FLAGS - vector with a series of binary flags indicating the type
%	   of the kernel used. Currently there are only two flags indicating:
%	   (1) the normalisation of the kernel; (2) its stationarity. Note
%	   that for some kernels these flags may be pointless (e.g. there is
%	   not a normalised version of the white kernel and it is always
%	   stationary). In these cases the kernel computation should simply
%	   ignore the flags. By default they are set to false.
%
%	KERN = KERNTEST(KERN, NUMIN, TIEPARAMNAMES, FLAGS) runs some tests
%	on a given kernel structure to ensure it is correctly implemented.
%	 Returns:
...
% 	kernTest.m SVN version 375
% 	last update 2009-06-02T22:01:40.000000Z
