...
% 	last update 2009-10-08T11:44:57.289410Z
...
switch call
    case 'click'
        [x, y, point]  = localCheckPointPosition(visualiseInfo);
        if ~isempty(x)
            fprintf('%g %g\n', x, y);
            if (isfield(visualiseInfo,'visualiseReevaluate'))
                visualiseInfo.latentPos(1) = x;
                visualiseInfo.latentPos(2) = y;
                fhandle = visualiseInfo.visualiseReevaluate;
                [val, val2] = fhandle(visualiseInfo.varargin{:}, visualiseInfo.latentPos);
%                 fprintf('val1 value: %g\n', val);
%                 fprintf('val2 value: %g\n', val2);
            end
        end
        ...
        if visualiseInfo.clicked & ~visualiseInfo.runDynamics
            ...
            if ~isempty(x)
                ...
                visualiseInfo.visualiseModify(visualiseInfo.visHandle, ...
                    Y, varsigma, visualiseInfo.latentPos, visualiseInfo.varargin{:});
            end
        end
    case 'toggleDynamics'
...
function [point pointua] = localGetNormCursorPoint(figHandle)
...
function [x, y, pointua] = localCheckPointPosition(visualiseInfo)

% Get the point of the cursor
[point pointua] = localGetNormCursorPoint(gcf);
