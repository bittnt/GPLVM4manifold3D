function nout=isonormals(varargin)
    ...
    %   $Revision: 1.8.4.4 $  $Date: 2011/04/04 22:59:07 $
    ...
    if isscalar(verts) && ishghandle(verts)
        if strcmp(get(verts, 'type'), 'patch')
            p = verts;
            verts = get(p, 'vertices');
        else
            error(id('InvalidPatchHandle'),'The handle supplied must be a patch handle');
        end
    else
        p = [];
    end
